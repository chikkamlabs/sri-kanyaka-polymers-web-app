...
function EditDiscountForm() {
  ...
  const [discount, setDiscount] = useState<DiscountWithDetails | null>(null);
  ...
  useEffect(() => {
    ...
    async function loadDiscountData() {
      ...
      if (!discountId) {
        if (mounted) {
          setErrorMessage('No Discount ID provided in URL parameters.');
          setLoading(false);
        }
        return;
      }
      ...
      try {
        const [foundDiscount, compList, catList] = await Promise.all([
        ...
        if (!foundDiscount) {
          if (mounted) {
            setErrorMessage(`Discount rule with ID "${discountId}" was not found.`);
            setLoading(false);
          }
          return;
        }
        ...
        if (mounted) {
          ...
          setCompanyId(foundDiscount.company_id || '');
          setCategoryId(foundDiscount.category_id || '');
          setD1(foundDiscount.d1 ?? 0);
          setD2(foundDiscount.d2 ?? 0);
          setD3(foundDiscount.d3 ?? 0);
          setD4(foundDiscount.d4 ?? 0);

          setLoading(false);
        }
      } catch (err: any) {
        if (mounted) {
          setErrorMessage(err.message || 'Failed to fetch discount rule details.');
          setLoading(false);
        }
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!discountId) {
      setErrorMessage('Missing Discount ID.');
      return;
    }
    ...
    try {
      // 1. Update discount rule in Supabase
      ...
      setErrorMessage(err.message || 'Failed to update discount rule in Supabase.');
      setSaving(false);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-[#F8F4EE] flex items-center justify-center p-4">
        <div className="bg-[#FFFCF8] border border-[#DDD3C6] rounded-2xl p-8 text-center max-w-sm w-full space-y-3 shadow-sm">
          ...
          <p className="text-xs font-semibold text-[#2F241E]">Loading Discount Rule...</p>
        </div>
      </div>
    );
  }
          ...
          <div className="bg-[#FFFCF8] border border-[#DDD3C6] rounded-2xl p-6 shadow-sm mb-6 flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-xs text-[#8A7B70] mt-0.5">
                Update discount values for {discount?.company_name || 'Company'} - {discount?.category_name || 'Category'}
              </p>
            </div>
          </div>
            ...
            <form onSubmit={handleSubmit} className="bg-[#FFFCF8] border border-[#DDD3C6] rounded-2xl p-6 sm:p-8 shadow-sm space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-emerald-700 mt-1 font-medium">Add discount (+)</p>
                </div>
              </div>
                ...
                >
                  {saving ? (
                    ...
                    <>
                      ...
                      <span>Update Discount Rule</span>
                    </>
                  )}
                </button>
              </div>
